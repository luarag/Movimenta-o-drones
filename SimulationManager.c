...
void sigint(int signum) {
    ...
    if (getpid() == simulation_pid) {
        ...
        while(list_cursor != NULL){ //percorre drones e envia sinal de saida
        ...
        while(list_cursor != NULL){ //percorre drones e destroi variaveis de condiçao e mutex
...
void *supply(){
    ...
    time (&rawtime); 
    struct tm *tm_struct = localtime(&rawtime);
    //enviar mensagens de abastecimento
    while(exit_requested == 0){
        sleep(freq_abast);
        w_no = round_robin_warehouse(w_no);  //armazem escolhido sequencialmente (round robin)
        do{                                    
            ind_prod = rand() % num_prod;
            strcpy(prod, product_types[ind_prod].type);
        }while(ind_product_warehouse(prod, w_no) == -1); //enquanto o produto encontrado nao se encontrar na warehouse 
...
int main()
{
    ...
    //CTRL-C terminaçao processos e threads
    signal(SIGINT, sigint);
    //Mostrar estatisiticas a pedido da central
    ...
    //create mutex to comunication between warehouse and central
    sem_unlink("MUTEX3");
    ...
    for(i=0;i<num_warehouses;i++){
        ...
        if (pid < 0) { 
            ...
            (shm_war+i)->w_no = i+1; //identificador unico de cada armazem
    ...
    //cria thread responsavel pelo abastecimento de produtos nos armazens 
